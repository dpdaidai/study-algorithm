...
#### 1.4.6 倍率实验
DoublingRatio可以简单有效的预测任意程序的性能, 并判断它们的运行时间大致增长数量级 
在有性能压力时应该考虑进行倍率实验 
###### 1) 评估它解决大型问题的可行性
了解程序的运行时间的增长数量级, 能够为程序提供精确信息 , 从而理解程序所能解决的问题规模的上限
###### 2) 评估使用更快的计算机所产生的价值

#### 1.4.7 注意事项
在对程序的性能进行仔细分析时, 有很多原因得到不一致或者是有误导性的结果
1.  大常数
2.  非决定性的内循环
3.  指令时间
4.  系统因素
...
#### 1.4.8 处理对输入的依赖
理想的状态下 , 我们希望根据算法能够得到清晰简洁的代码, 并能够为我们感兴趣的输入提供良好的性能和保证
###### 1) 输入模型
###### 2) 对最坏的情况下性能的保证
###### 3) 随机化算法
###### 4) 操作序列
...
#### 1.4.9 内存
Java最重要的特质之一就是内存分配系统 . 程序员应该直到程序对内存的需求在何时会称为解决问题的障碍
###### 1) 对象
###### 2) 链表
###### 3) 数组
###### 4) 字符串对象
