...
#### 3.1.5 有序数组中的二分查找
使用一对平行的数组, 一个存储键一个存储值 , 保证数组中Comparable类型的键有序 , 然后使用
数组的索引来高效的实现get()操作
1.  实现的核心是rank()方法 , 它返回表中小于给定键的键数 , 它将使用二分查找
    + 表中存在给定的key , rank()返回该key的位置 , 也就是小于key的键的数量
    + 表中不存在key , rank()还是会返回小于它的键的数量
    + 二分查找结束时 , lo正好等于被查找的键的数量
2.  对于get()方法, 只要给定的键存在于表中, rank()方法就能精确的告诉我们在哪里找到他
3.  对于put()方法, rank()方法也能快速找到它的位置 , 没有的话也能直到在哪里插入新的键值
    + 插入时, 我们将所有更大的键向后移动一个来腾出位置, 并将给定的键值插入合适的位置
    + 可以动态调整数组大小 , 但是对于大数组非常慢

基于有序数组的符号表运行时间 : 
1.  有N个键的情况下, 二分查找最多需要 lgN+1 次比较 , 无论是否命中
2.  N个键时 , put() 和 delete() 最坏情况需要访问 2N次数组 . 
    + 例如在0处插入和删除 , 分别需要移动 keys 和 values 两个数组中的所有元素
3.  那么向空符号表中插入N个元素 , 平均需要访问 ~N² 次数组

#### 3.1.7 预览
简单符号表成本总结 : 操作成本 = 比较次数 + 访问数组元素次数
1.  顺序查找(无序链表) 
    + 最坏情况 : 查找和插入都是 N 次操作 
    + 平均情况 : 查找 N/2 , 插入 N 次操作
    + 不支持高效搜索
    + 不支持有序性操作
2.  二分查找(有序数组) 
    + 最坏情况 : 查找 lgN 次比较 , 插入2N 次访问数组
    + 平均情况 : 查找 lgN 次比较 , 插入 N 次访问数组
    + 支持高效搜索
    + 支持有序性操作
    
我们需要能够同时支持高效搜索和高效插入的数据结构 , 各符号表的优缺点 :
1.  链表
    1.  优点 : 适用小型问题
    2.  缺点 : 大型符号表很慢 
2.  有序数组 
    1.  优点 : 最优查找效率和空间需求 , 可进行有序性操作
    2.  缺点 : 插入操作很慢
3.  二分查找树
    1.  优点 : 实现简单 , 能进行有序性操作
    2.  缺点 : 没有性能上界 , 链接需要额外空间
4.  平衡二叉查找树
    1.  优点 : 最优的查找和插入效率, 能进行有序性操作
    2.  缺点 : 链接需要额外空间
5.  散列表
    1.  优点 : 能快速的查找和插入常见类型的数据
    2.  缺点 : 
        +   需要进行每种数据的散列
        +   无法进行有序性操作
        +   链接和空节点需要额外空间
